#AUTONAVx Homework 7 Programming Assignment
#Script for implementing an Extended Kalman Filter
#The vast majority of this program was already written, the "correctState" method and
#"calculatePredictMeasurementJacobian" were implemented as part of the assignment.
#
#This program runs on a simulator, with the goal of tracing a particular path by
#estimating the position of the quadrotor by integrating the acceleration measurements
#from the onboard IMU. These integrations get increasingly innacurate with time due to
#noise in the system. The innacuracies can be counteracted by using the onboard camera
#to identify known markers in the world and taking these points as new initial
#positions. This way the robot is only integrating accelerations for a short period of
#time in between visual markers.
...
class UserCode:
    ...
    def correctState(self, K, x_predicted, z, z_predicted):
        ...
        temp_z = z - z_predicted
        temp_z[2] = self.normalizeYaw(temp_z[2])
        x_predicted = x_predicted + np.dot(K, temp_z)
    ...
    def calculatePredictMeasurementJacobian(self, x, marker_position_world, marker_yaw_world):
        ...
        psi = x[2]
        m_x = marker_position_world[0]
        m_y = marker_position_world[1]
        q_x = x[0]
        q_y = x[1]
              
        H = np.array([[-1*math.cos(psi), -1*math.sin(psi), ((-1*math.sin(psi)*(m_x - q_x)) + (math.cos(psi)*(m_y - q_y)))],
              [math.sin(psi), -1*math.cos(psi), ((-1*math.cos(psi)*(m_x - q_x)) + (-1*math.sin(psi)*(m_y - q_y)))],
              [0, 0, -1]])
